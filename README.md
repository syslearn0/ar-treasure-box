...
コマンドは**上から順に、1つずつ**実行してください。
1行ごとに Enter を押して、終わってから次に進みます。

### 0. ターミナルを開く

| | |
|---|---|
| Windows | スタートメニューを開いて `PowerShell` と打ち、出てきたものを起動 |
| Mac | `command + スペース` → `ターミナル` と打って起動 |

以降、`$` や `>` のような行頭の記号は打ちません。**枠の中身だけ**をコピーします。
...
どちらもバージョン番号（`git version 2.x.x` や `v22.x.x` のような文字列）が出れば大丈夫です。
`コマンドが見つかりません` と出たら、先にこちらを入れてください。

- Git → https://git-scm.com/downloads
- Node.js → https://nodejs.org （**LTS** と書いてあるほう）

インストール後は、**ターミナルを一度閉じて開き直して**からもう一度確かめます。
開いたままだと新しく入れたものが見つかりません。
...
`Documents` の中に置く例です。別の場所がよければそのフォルダに読み替えてください。

### 3. ダウンロードする（クローン）

```bash
git clone https://github.com/syslearn0/ar-treasure-box.git
```

`ar-treasure-box` という名前のフォルダができます。
...
ターミナルを開き直したときは、まず 2 と 4 をもう一度実行してください。
...
（`npm warn` は警告なので無視して構いません。`npm error` が出たら失敗です）
...
**止めるときは `Ctrl + C`**（Mac も `control + C`）です。ウィンドウを閉じても止まります。

---

## 最新版に更新する

こちらで直したものを取り込むときは、これだけです。

```bash
cd ~/Documents/ar-treasure-box
```
...
`npm install` は、必要なものが増えていた場合に備えて毎回打っておくと安全です。
変わっていなければ数秒で終わります。

### 「変更があるので pull できない」と言われたら

自分で書き換えたファイルがあると、`git pull` は上書きを避けて止まります。
まず何が変わっているかを見ます。
...
**自分の変更を残したまま更新する場合** — 一時的に退避して、更新して、戻します。
...
**自分の変更を捨てて、配布された状態に戻す場合** — ⚠️ 書き換えた内容は消えます。
...
`git push` は使いません。手元で好きなだけ書き換えて構いませんし、
それがこちらのリポジトリに影響することもありません。

---

## うまくいかないとき

**スマホ・タブレットだけ開けない（PCでは開ける）** — 出る症状は「安全な接続を確立できません」など。

開発用の HTTPS は自己署名証明書です。この証明書には**この PC のローカル IP が対象として入って**います
（`scripts/dev-cert.mjs` が起動時に作ります）。それでも「発行元が信頼できない」警告は出るので、
**一度だけ許可**してください。

- iOS Safari … 「詳細を表示」→「このWebサイトを閲覧」
- Android Chrome … 「詳細設定」→「（安全ではありません）にアクセスする」

ポートが変わると Safari は別サイトとして扱うので、**ポートごとに一度ずつ**許可が要ります。

> **なぜ自前で証明書を作っているか** — `@vitejs/plugin-basic-ssl` が作る証明書は
> 対象が `localhost` と `127.0.0.1` だけで、LAN の IP が入りません。
> PC のブラウザは名前不一致の警告を押し通せますが、iOS Safari は通しません。
> 実際、iPad からの TLS 握手は `certificate unknown`（alert 46）で落ちていました。
> 証明書は IP の顔ぶれが変わると自動で作り直されるので、テザリングと自宅 Wi-Fi を
> 行き来しても追従します。
...
**`npm install` が途中で失敗する** — 入れかけのものを消してやり直します。
...
そのうえで、配布時と同じ内容で入れ直します。

```bash
npm ci
```

**`cd` したのに違う場所にいる気がする** — いまどこにいるかを表示します。
...
**もう一度まっさらからやり直したい** — フォルダごと消して、手順 2 からやり直すのが確実です。
消える前に、自分で作った `.glb` や画像は別の場所に退避しておいてください。
...
`src/config.ts` の `SHOW_GUIDES` を `true` にすると、座標軸・追跡領域の枠・
向きを示す矢印・オレンジの立方体が出る。空間に固定できているかを目で確かめるためのもの。

**既定は `false`。** キャラクターと重なるので、ふつうは出さない。
書き換えずに一時的に出したいときは URL に付ける。

```
https://…:5173/?guides=1
```

同じ要領で `?debug=0` と付ければ、画面左上の数値表示を消せる。

---

## カメラ映像の収め方

`src/config.ts` の `CAMERA_FIT` で決める。

| 値 | 見え方 |
|---|---|
| `'contain'`（既定） | 映像の全体を**等倍**で映す。上下（または左右）に黒帯が出る |
| `'cover'` | 画面いっぱいに広げる。はみ出した部分は切り落とされる |

8th Wall の映像描画はキャンバスを覆うように拡大して中央で切り取る（エンジンの実装で確認）。
スマホの画面は縦長でカメラは 4:3 なので、画面いっぱいにすると左右が切られ
**1.5〜1.6倍に寄った映像**になり、カードを画面に収めにくくなる。
そこでキャンバスの縦横比を映像に一致させ、切り取り量を 0 にしている。

---

## 各コマンドの意味

`npm install` と起動までは上の「手元に持ってくる」で済んでいます。
ここは各コマンドが何をするかの説明です。
...
`@8thwall/engine-binary` が入る。各コマンドの先頭で
